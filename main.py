...
def register():
    ...
    if form.validate_on_submit():
        ...
        data = werkzeug.security.generate_password_hash(password, method='pbkdf2:sha256', salt_length=8)
        new_user = User(
            name=name,
            email=email,
            password=data,
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            data = werkzeug.security.check_password_hash(password_hash, password)
            if data:
                login_user(user)
...
def show_post(post_id):
    ...
    return render_template("post.html", post=requested_post, current_user=current_user, user=user, form=form,
